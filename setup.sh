...
build_project() {
    ...
    case "$target" in
        client)
            info "Building space_lt_client..."
            cargo build --release -p space_lt_client
            sudo cp "$dir/target/release/space_lt_client" /usr/local/bin/
            info "Installed space_lt_client to /usr/local/bin/"
            ;;
        server)
            local build_flags="--release -p space_lt_server"
            if [ "${CUDA_ENABLED:-0}" = "1" ]; then
                build_flags="$build_flags --features cuda"
            fi
            info "Building space_lt_server ($build_flags)..."
            cargo build $build_flags
            sudo cp "$dir/target/release/space_lt_server" /usr/local/bin/
            info "Installed space_lt_server to /usr/local/bin/"
            ;;
        *)
            local build_flags="--release --workspace"
            if [ "${CUDA_ENABLED:-0}" = "1" ]; then
                build_flags="$build_flags --features space_lt_server/cuda"
            fi
